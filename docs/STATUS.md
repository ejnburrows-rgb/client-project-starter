...
- GitHub repository slug remains `client-project-starter`; rename only when repository-settings write access is intentionally used.
- The nested compatibility copy can be removed only after a full Vercel deployment serves the root build directly and production is verified.
- A real owner-approved contact or scheduling destination is still required before the public review CTA can submit or book anything.
